...
pairs(~beta0+beta1+beta_mod_A+rho+V_wane_rate+V_fail_rate+rec_rate,data=data,col="grey",pch=16,cex=.4)
pairs(~S_0+I_0+V_0+A_0,data=data,col="grey",pch=16,cex=.4)

## plot initial results
setwd("~/Dropbox (Princeton)/USPertussis/pomp/results/initial results")
data<-read.csv(paste0("initial.sweep.",job.name,".csv"))
pairs(~beta0+beta1+beta_mod_A+rho+V_wane_rate+V_fail_rate+rec_rate,data=data,col=ifelse((data$loglik) > quantile(data$loglik,.9)[[1]],rgb(1,0,0,alpha=.5),rgb(.6,.6,.6,alpha=.5)),pch=16,cex=.4)
pairs(~S_0+I_0+V_0+A_0,data=data,col=ifelse((data$loglik) > quantile(data$loglik,.9)[[1]],rgb(1,0,0,alpha=.5),rgb(.6,.6,.6,alpha=.5)),pch=16,cex=.4)
     
## plot final results
setwd("~/Dropbox (Princeton)/USPertussis/pomp/results/final results")
data<-read.csv(paste0("final.sweep.",job.name,".csv"))
pairs(~beta0+beta1+beta_mod_A+rho+V_wane_rate+V_fail_rate+rec_rate,data=data,col=ifelse((data$loglik) > quantile(data$loglik,.9)[[1]],"red","grey"),pch=16,cex=.4)
pairs(~S_0+I_0+V_0+A_0,data=data,col=ifelse((data$loglik) > quantile(data$loglik,.9)[[1]],"red","grey"),pch=16,cex=.4)
...
setwd("~/Dropbox (Princeton)/USPertussis/pomp/results/LHS")
data<-readRDS(paste0(job.name,".LHS.RDS"))
plot(data$beta_mod_A,data$V_fail_rate,col="grey",pch=16)

setwd("~/Dropbox (Princeton)/USPertussis/pomp/results/initial results")
data<-read.csv(paste0("initial.sweep.",job.name,".csv"))
data<-subset(data,loglik>=quantile(data$loglik,.95))
points(data$beta_mod_A,data$V_fail_rate,col="red",pch=16,cex=(data$loglik-min(data$loglik))/(max(data$loglik)-min(data$loglik)))


setwd("~/Dropbox (Princeton)/USPertussis/pomp/results/final results")
data<-read.csv(paste0("final.sweep.",job.name,".csv"))
data<-subset(data,loglik>=quantile(data$loglik,.95))
points(data$beta_mod_A,data$V_fail_rate,col="blue",pch=16,cex=(data$loglik-min(data$loglik))/(max(data$loglik)-min(data$loglik)))
...
setwd("~/Downloads")
source("~/Dropbox (Princeton)/USPertussis/pomp/cluster/code/build.pomp.all.equal.R")

setwd("~/Dropbox (Princeton)/USPertussis/pomp/results/final results")
data<-read.csv(paste0("final.sweep.",job.name,".csv"))
params<-data[order(-data$loglik)[1],8:21]
