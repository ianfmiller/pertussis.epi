...
statenames <- c("S","E","En","Ev","I","A","An","Av","Vn","V","W","C")
t0 <- min(dates)

init.param.names<-c("S_0","E_0","En_0","Ev_0","I_0","A_0","An_0","Av_0","Vn_0","V_0")
param.names<-c("Vn_wane_rate","V_wane_rate","Vn_fail_rate","Vn_symptom_rate","V_fail_rate","V_symptom_rate","naive_symptom_rate","beta0","beta1","beta_mod_An","beta_mod_Av","beta_mod_A","rho","sigmaSE","lag",init.param.names)

rproc <- Csnippet("
                  double N, beta, latent_trans_rate, rec_rate, foi, dw, nbirths, Sbirths, Vwp_births, Vap_births;
                  double rate[25], trans[25];
                  ...
                  // Ev
                  rate[8] = (1-V_symptom_rate)*latent_trans_rate; // rate of transition from Ev to Av
                  rate[9] = V_symptom_rate*latent_trans_rate; // rate of transition from Ev to I
                  ...
                  // V
                  rate[22] = V_fail_rate*foi*dw/dt; // rate at which V becomes Ev
                  rate[23] = V_wane_rate;         // V waning rate
                  ...
                  reulermultinom(3, V, &rate[22], dt, &trans[22]);

                  S += Sbirths + trans[20] + trans[23] - trans[0] - trans[1];
                  E += trans[0] - trans[2] - trans[3] - trans[4];
                  En += trans[19] - trans[5] - trans[6] - trans[7];
                  Ev += trans[22] - trans[8] - trans[9] - trans[10];
                  ...
                  V += Vwp_births + Vap_births - trans[22] - trans[23] - trans[24];
...
rinit <- Csnippet("
                  double m = pop/(S_0+E_0+En_0+Ev_0+I_0+A_0+An_0+Av_0+Vn_0+V_0);
                  S = nearbyint(m*S_0);
                  E = nearbyint(m*E_0);
                  En = nearbyint(En_0);
                  Ev = nearbyint(m*Ev_0);
...
dmeas <- Csnippet("
                  ...
                  //if (!R_FINITE(lik)) {Rprintf(\"%lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg %lg\\n\",S,E,En,Ev,I,A,An,Av,Vn,V,beta0,beta1,lag,lik,C);}
                  ")
...
partrans=parameter_trans(
  log=c("Vn_wane_rate","V_wane_rate","beta0","sigmaSE"),
  logit=c("naive_symptom_rate","Vn_fail_rate","Vn_symptom_rate","V_fail_rate","V_symptom_rate","beta_mod_A","beta_mod_An","beta_mod_Av","rho","lag","beta1"),
  barycentric=c("S_0","E_0","En_0","Ev_0","I_0","Vn_0","V_0","A","An_0","Av_0")
)
...
est.pars<-c("Vn_wane_rate","V_wane_rate","Vn_fail_rate","Vn_symptom_rate","V_fail_rate","V_symptom_rate","naive_symptom_rate",
            "beta0","beta1","beta_mod_An","beta_mod_Av","beta_mod_A","rho","sigmaSE","lag",
            "S_0","E_0","En_0","Ev_0","I_0","Vn_0","V_0","A","An_0","Av_0")

rw.sd=rw.sd(Vn_wane_rate=0.01,V_wane_rate=0.01,Vn_fail_rate=0.01,Vn_symptom_rate=0.01,V_fail_rate=0.01,V_symptom_rate=0.01,naive_symptom_rate=0.01,
            beta0=0.01,beta1=0.01,beta_mod_An=0.01,beta_mod_Av=0.01,beta_mod_A=0.01,rho=0.01,sigmaSE=0.01,lag=0.01,
            S_0=ivp(0.01),E_0=ivp(0.01),En_0=ivp(0.01),Ev_0=ivp(0.01),I_0=ivp(0.01),A_0=ivp(0.01),An_0=ivp(0.01),Av_0=ivp(0.01),Vn_0=ivp(0.01),V_0=ivp(0.01))
...
init.values.mat<-data.frame(S_0=LHS[,"S_0"],
                            ...
                            Vn_0=LHS[,"Vn_0"],V_0=LHS[,"V_0"])

params.mat<-cbind(
  data.frame(
    Vn_wane_rate=LHS[,"Vn_wane_rate"],V_wane_rate = LHS[,"V_wane_rate"], Vn_fail_rate=LHS[,"Vn_fail_rate"],Vn_symptom_rate=LHS[,"Vn_symptom_rate"],
    V_fail_rate=LHS[,"V_fail_rate"],V_symptom_rate=LHS[,"V_symptom_rate"],naive_symptom_rate=LHS[,"naive_symptom_rate"],
