...
statenames <- c("S","I","Vwp","Vnap","Awp","Anap","W","C")
t0 <- min(dates)

init.names<-c("S_0","I_0","Vwp_0","Vnap_0","Awp_0","Anap_0")
param.names<-c("Vwp_wane_rate","Vnap_wane_rate","rec_rate","Vwp_fail_rate","Vwp_symptom_rate","Vnap_fail_rate","Vnap_symptom_rate","beta0","beta1","beta_mod_Awp","beta_mod_Anap","rho","sigmaSE","lag",init.names)
...
rproc <- Csnippet("
                  ...
                  //Vnap
                  rate[8] = Vnap_fail_rate*Vnap_symptom_rate*foi*dw/dt; // rate at which Vnap becomes I
                  rate[9] = Vnap_fail_rate*(1.0-Vnap_symptom_rate)*foi*dw/dt; // rate at which Vnap becomes Anap
                  rate[10] = Vnap_wane_rate;         // Vnap waning rate
                  ...
                  //Anap
                  rate[14] = rec_rate;   // Anap recovery rate
                  ...
                  reulermultinom(4, Vnap, &rate[8], dt, &trans[8]);
                  reulermultinom(2, Awp, &rate[12], dt, &trans[12]);
                  reulermultinom(2, Anap, &rate[14], dt, &trans[14]);
                  ...
                  Vnap += Vap_births + trans[2] + trans[12] + trans[14] - trans[8] - trans[9] - trans[10] - trans[11];
                  Awp += trans[5] - trans[12] - trans[13];
                  Anap += trans[9] - trans[14] - trans[15];
...
rinit <- Csnippet("
                  double m = pop/(S_0+I_0+Vwp_0+Vnap_0+Awp_0+Anap_0);
                  S = nearbyint(m*S_0);
                  I = nearbyint(m*I_0);
                  Vwp = nearbyint(m*Vwp_0);
                  Vnap = nearbyint(m*Vnap_0);
...
est.pars<-c("Vwp_wane_rate","Vnap_wane_rate","rec_rate","beta0","sigmaSE",
            "Vwp_fail_rate","Vwp_symptom_rate","Vnap_fail_rate","Vnap_symptom_rate",
            "beta_mod_Awp","beta_mod_Anap","rho","lag","beta1","S_0","I_0","Vwp_0","Vnap_0","Awp_0","Anap_0")

rw.sd=rw.sd(Vwp_wane_rate=0.02,Vnap_wane_rate=0.02,rec_rate=0.02,beta0=0.02,
            ...
            rho=0.02,lag=0.02,beta1=0.02,S_0=0.02,I_0=0.02,Vwp_0=0.02,Vnap_0=0.02,Awp_0=0.02,Anap_0=0.02)

init.values.mat<-data.frame(S_0=LHS[,"S_0"],I_0=LHS[,"I_0"],Vwp_0=LHS[,"Vwp_0"],Vnap_0=LHS[,"Vnap_0"],Awp_0=LHS[,"Awp_0"],Anap_0=LHS[,"Anap_0"])

params.mat<-cbind(data.frame(Vwp_wane_rate = LHS[,"Vwp_wane_rate"],Vnap_wane_rate = LHS[,"Vnap_wane_rate"], rec_rate = LHS[,"rec_rate"],
