...
n.initial<-8000 #number of LHS samples
n.final<-800 #n best samples to analyze further
...
foreach(i=0:(jobs.per.node-1), .inorder=F, .combine = "rbind") %dorng% {
  ...
  m1<-m1$mif
    continue(m1,Nmif=100,rw.sd=rw.sd,cooling.fraction.50=0.2, Np=1000,cooling.type="hyperbolic") %>%
    continue(Nmif=100,rw.sd=rw.sd,cooling.fraction.50=0.1, Np=1000,cooling.type="hyperbolic") %>%
    continue(Nmif=100,rw.sd=rw.sd,cooling.fraction.50=0.05, Np=1000,cooling.type="hyperbolic")->m2
