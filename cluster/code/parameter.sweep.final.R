...
foreach(i=0:(jobs.per.node-1), .inorder=F, .combine = "rbind") %dorng% {
  ...
  m1<-m1$mif
    continue(m1,Nmif=5,rw.sd=rw.sd,cooling.fraction.50=0.5, Np=25,cooling.type="hyperbolic") %>%
    continue(Nmif=7,rw.sd=rw.sd,cooling.fraction.50=0.25, Np=25,cooling.type="hyperbolic") %>%
    continue(Nmif=7,rw.sd=rw.sd,cooling.fraction.50=0.1, Np=25,cooling.type="hyperbolic")->m2
